...
export default function Testimonials() {
  ...
  return (
    <div className="w-full mx-auto px-4 sm:px-6 lg:px-0 mt-5">
          ...
          <div className="rounded-lg p-6 max-w-sm mx-auto">
            <div className="text-center">
              ...
              <div className="flex justify-center mb-2">
                <div className="flex space-x-1" aria-label="4.5 out of 5 stars">
                  {[1, 2, 3, 4].map((star) => (
                    <FaStar key={star} className="w-5 h-5 text-orange-400" />
                  ))}
                  <FaStar className="w-5 h-5 text-orange-400 opacity-50" />
                </div>
              </div>
              <p className="text-gray-600 mb-3">
                Based on <span className="font-bold">500+ Reviews</span>
              </p>
      ...
      <div className="relative w-full py-8 px-4 md:px-1">
        <div className="relative p-5">
          ...
          >
            {reviews.map((item) => (
              <SwiperSlide key={item.id || item._id || item.name} className="!flex justify-center px-4">
                <TestimonialCard
                  name={item.name}
          ...
          <button type="button" aria-label="Previous testimonial" className="swiper-button-prev-custom absolute left-2 top-[45%] z-10 text-green-900">
            <FaChevronLeft size={24} />
          </button>
          <button type="button" aria-label="Next testimonial" className="swiper-button-next-custom absolute right-2 top-[45%] z-10 text-green-900">
        ...
        <div className="flex justify-center mt-10">
          <Link
            href="https://www.google.com/maps/place/CROWN+EXCEL+(Experience+Center)/@25.2603141,55.2886443,17z/data=!4m8!3m7!1s0x3e5f43ba6913e913:0x904de2fef7d413ec!8m2!3d25.2603093!4d55.2912192!9m1!1b1!16s%2Fg%2F11j7jbshl8?entry=ttu&g_ep=EgoyMDI1MDgxOS4wIKXMDSoASAFQAw%3D%3D"
            target="_blank"
            rel="noopener noreferrer"
            className="px-8 py-4 cursor-pointer bg-gradient-to-r from-[#16CA9A] to-[#084032] text-white font-bold rounded-xl shadow-lg hover:bg-[#084032] transition-colors duration-300"
          >
            Add a Review
          </Link>
        </div>
