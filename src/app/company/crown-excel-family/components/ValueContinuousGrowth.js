...
export default function ValueContinuousGrowth({ title, desc }) {
  return (
    <div className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col items-center justify-center px-8 py-10 border border-[#e5e7eb]" style={{ minHeight: '260px' }}>
      ...
      <div className="flex flex-col items-center text-center">
        <h3 className="text-2xl font-bold text-gray-900 mb-2 font-montserrat">{safeRender(title, "Continuous Growth")}</h3>
        <p className="text-gray-500 font-muli text-base leading-relaxed mb-2">{safeRender(desc, "Continuous Growth Embracing change and fostering continuous learning and development.")}</p>
      </div>
    </div>
  );
}
