...
export default function MemberShammry({ name, position, tenure, dept, quote }) {
  const displayName = name || "Shamry Salam";
  const displayPosition = position || "Receptionist";
  const displayTenure = tenure || "10+ Years";
  const displayDept = dept || "Reception";
