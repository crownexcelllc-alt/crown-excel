...
export default function MemberJunaidS({ name, position, tenure, dept, quote, image }) {
  const displayImage = image || etc5;
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 group">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold text-[#16806b] font-montserrat mb-2 group-hover:text-[#084032] transition-colors">{safeRender(name, "Junaid Shafique")}</h3>
        <p className="text-[#084032] font-muli font-medium mb-4">{safeRender(position, "Accounts Manager")}</p>
