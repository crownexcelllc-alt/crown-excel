...
export default function MemberVishal({ name, position, tenure, dept, quote, image }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 group">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold text-[#16806b] font-montserrat mb-2 group-hover:text-[#084032] transition-colors">{safeRender(name, "Vishal Pawar")}</h3>
        ...
        <blockquote className="text-gray-600 font-muli italic text-sm leading-relaxed border-l-4 border-[#61ce70] pl-4">
          {quote ? (
            ...
            <p className="text-gray-600 font-muli italic text-sm">
              "{safeRender(null, "Building lasting relationships is at the heart of everything we do.")}"
            </p>
          )}
        </blockquote>
      </div>
