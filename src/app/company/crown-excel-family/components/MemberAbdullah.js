...
export default function MemberAbdullah({ name, position, tenure, dept, quote, image }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 group">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold text-[#16806b] font-montserrat mb-2 group-hover:text-[#084032] transition-colors">{safeRender(name, "Abdullah Salar")}</h3>
        <p className="text-[#084032] font-muli font-medium mb-4">{safeRender(position, "Purchasing Officer")}</p>
