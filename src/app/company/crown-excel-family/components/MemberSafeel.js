...
export default function MemberSafeel({ name, position, tenure, dept, quote, image }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 group">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold text-[#16806b] font-montserrat mb-2 group-hover:text-[#084032] transition-colors">{safeRender(name, "Mohamed Safeel")}</h3>
        <p className="text-[#084032] font-muli font-medium mb-4">{safeRender(position, "Accounts & Logistics")}</p>
        <blockquote className="text-gray-600 font-muli italic text-sm leading-relaxed border-l-4 border-[#61ce70] pl-4">
          {quote ? (
            ...
            <p className="text-gray-600 font-muli italic text-sm">
              "{safeRender(null, "Dedicated to providing the best service to our customers.")}"
            </p>
          )}
        </blockquote>
      </div>
